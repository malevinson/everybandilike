...
function ShareController($rootScope, $auth, $uibModal, RatingService, SpotifyService, Storage, toastr, $stateParams, $aside, $location) {
    ...
    function initialize(){
        ...
        // Hardcoded mock artists (for testing - remove when DB is working)
        var mockArtists = [
            {
                artist: {
                    name: 'Tom Petty',
            ...
            },
            {
                artist: {
                    name: 'Taylor Swift',
                    spotifyId: '06HL4z0CvFAxyc27GXpf0g2',
                    picture: 'https://i.scdn.co/image/ab6761610000f178aa69380a8d5203257611662d',
                    genres: ['pop', 'country', 'pop country']
                },
                ratingGiven: 1
            }
        ];

        // Add mock artists immediately
        ...
        RatingService
            ...
            .catch(function(err) {
                console.log('Could not fetch latest collections:', err);
            });
        ...
        if ($auth.provider.isAuthenticated()) {
            ...
            RatingService
                ...
                .then(function (result) {
                    console.log('🔵 DB API result (authenticated):', result);
                    console.log('🔵 Result type:', typeof result, 'Is array:', Array.isArray(result), 'Length:', result ? result.length : 'null');
                    // Clear mock data if we get real data from DB
                    if (result && Array.isArray(result) && result.length > 0) {
                        console.log('🔵 Clearing mock data, using DB data');
                        self.artists = { one: [], two: [], three: [] };
                        result.forEach(function (el) {
                            self.artists[arrayName(el.ratingGiven)].push(el);
                        });
                    } else {
                        console.log('🔵 No DB data or empty result, keeping mock data');
                    }
                    self.loaded = true;
                })
                .catch(function(err) {
                    console.error('🔴 DB API error (authenticated):', err);
            ...
            console.log('🔵 Attempting to fetch DB data for hash:', $stateParams.hash);
            RatingService
                .get($stateParams.hash)
                .then(function (result) {
                    console.log('🔵 DB API result (viewer):', result);
                    console.log('🔵 Result type:', typeof result, 'Is array:', Array.isArray(result), 'Length:', result ? result.length : 'null');
                    // Clear mock data if we get real data from DB
                    if (result && Array.isArray(result) && result.length > 0) {
                        console.log('🔵 Clearing mock data, using DB data');
                ...
                .catch(function(err) {
                    console.error('🔴 DB API error (viewer):', err);
                    console.log('🔵 Keeping mock data due to error');
                    self.loaded = true;
                });
